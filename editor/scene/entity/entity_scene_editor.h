...
class EntitySceneEditor : public VBoxContainer {
	...
	bool number_pending = false;
	Variant pending_value;
	uint64_t pending_component = 0;
	uint64_t pending_field = 0;
	Array pending_path;
	...
	void _inspect();
	void _add_field(const String &p_label, const Variant &p_value, uint64_t p_component, uint64_t p_field, const Array &p_path, const EntityFieldSchema *p_schema = nullptr);
	void _change(const Variant &p_value, uint64_t p_component, uint64_t p_field, const Array &p_path);
	void _number_changed(double p_value, uint64_t p_component, uint64_t p_field, Array p_path, bool p_integer);
	void _text_changed(const String &p_value, uint64_t p_component, uint64_t p_field, Array p_path);
...
public:
	...
	void commit_pending_edits() { _flush_number(); }
